...
async def orm_add_action_with_ideas(session: AsyncSession, user_id: int, user_name: str, idea: str, for_who: int):
    try:
        new_action = Ideas(
            user_id=user_id,
            user_name=user_name,
...
# Функция для добавления идей пользвателю
async def orm_add_action_with_question(session: AsyncSession, user_id: int, message_id:int, user_name: str, question: str, for_who: int):
    try:
